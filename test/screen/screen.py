...
font2 = ImageFont.truetype('/opt/stratux/data/CnC_Red_Alert.ttf', 12)
serial = i2c(port=0, address=0x3c)
oled = ssd1306(serial)

with canvas(oled) as draw:
    logo = Image.open('/opt/stratux/data/stratux-logo-64x64.bmp')
...
while 1:
    time.sleep(1)
    response = urllib.request.urlopen('http://localhost/getStatus')
    ...
    response = urllib.request.urlopen('http://localhost/getTowers')
